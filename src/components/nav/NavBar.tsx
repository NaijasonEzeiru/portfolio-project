...
const NavBar = () => {
  ...
  return (
    <header
      className='bg-gray-100 flex items-center justify-between px-3 text-sm py-2 md:px-14 lg:px-32 text-black'
      ref={navRef}>
      <Link
        href={'/'}
      ...
      <nav
        className={`flex gap-2 w-full left-0 absolute transition-transform z-50 bg-gray-100 flex-col py-8 px-2 top-11 md:w-auto md:static md:bg-transparent md:flex-row md:translate-x-0 md:py-0  ${
      ...
      <div className='flex gap-3 items-center'>
        ...
        <Link href={'/profile'} className='text-inherit contents'>
          <button
            ...
            className='group relative cursor-pointer p-1 rounded-full border-solid border-[1px] border-black'>
            <svg
              xmlns='http://www.w3.org/2000/svg'
              aria-hidden
              fill='none'
      ...
      <span
        ...
        <span className='hidden group-focus:block group-hover:block bg-secondary text-white rounded-[4px] py-[1px] px-[3px] text-xs text-center z-50 absolute top-[110%] left-0 after:content-[""] after:absolute after:bottom-full after:left-[40%] after:-ml[5px] after:border-[5px] after:border-solid after:border-b-secondary after:border-x-transparent after:border-t-transparent'>
          Menu
        </span>
      </span>
    </header>
