...
export const EditProfileSchema = z
  .object({
    ...
    email: z
      ...
      .max(40, { message: 'Must contain at most 40 characters' }),
    phone: z.coerce.number(),
    displayImg: z.string().nullable(),
    newImg: z.any().nullable()
    // .refine(
    //   (file) => file?.[0]?.size <= MAX_FILE_SIZE,
    //   'Max image size is 5MB.'
    // )
  })
  .superRefine(({ newImg }, ctx) => {
    if (newImg?.[0]?.size && newImg?.[0]?.size >= MAX_FILE_SIZE) {
      ctx.addIssue({
        code: 'custom',
...
export const lastStepSchema = z.object({
  ...
  negotiable: z.string().min(2, { message: 'Select the condition' }).optional(),
